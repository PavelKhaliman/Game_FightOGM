...
рисованными 2D-персонажами и интерфейсом на русском. Доступны **10 бойцов**:
Иванов, Гаглоев, Киселик, Новацкий, Жирнов, Елхимов, Калачев, Федосеев, Халиман и Шуев.
Можно играть против компьютера или вдвоём на одной клавиатуре, выбирая любую пару.

![Выбор десяти бойцов в FIGHTOGM 2D](docs/images/character-select.png)
...
| Новацкий | Шнек | Я занят. | Партия шнеков |
...
Шнек, робот, вода, волна сарказма и электрический импульс имеют отдельные 2D-эффекты.

## Что изменено в 2D

- Рисованные спрайты всех десятерых: 360 отдельных поз и 171 игровая анимация.
...
```powershell
.\dist\FightOGM.exe -assets-root D:\FightOGM\dist -smoke-test -output D:\FightOGM\test-output\2d-roster-dist
```

Она проверяет 360 кадров, удаление зелёного фона шейдером, выбор по сетке, оба режима
игры, зеркальный бой каждого персонажа, все 30 особых действий, пять типов снарядов,
обычные удары, блок, прыжок, полный матч, паузу и реванш. Сохраняет PNG-кадры
и report.json. Модульные тесты также проверяют бой компьютера за всех десятерых.
...
- assets/sprites/<id>/ — по три листа (movement, attacks, actions) и animations.json
  для каждого из девяти новых бойцов.
...
- scripts/sprites/build_roster.py — построение JSON для девяти новых бойцов.
...
и запись в roster.json. Все десять определений из internal/characters включены
